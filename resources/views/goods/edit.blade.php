...
<center>
    <form action="{{url('goods/update/'.$goodsInfo->goods_id)}}" method="post" enctype="multipart/form-data">
        ...
        <table>
            <tr>
                <td>商品名称</td>
                <td><input type="text" name="goods_name"  value="{{$goodsInfo->goods_name}}"><b style="color:red">{{ $errors->first('goods_name') }}
                    </b></td>
            </tr>
            <tr>
                <td>商品货号</td>
                <td><input type="text" name="goods_item" value="{{$goodsInfo->goods_item}}"><b style="color:red">{{ $errors->first('goods_item') }}</b></td>
            </tr>
            <tr>
                <td>商品价格</td>
                <td><input type="text" name="goods_price"  value="{{$goodsInfo->goods_price}}"><b style="color:red">{{ $errors->first('goods_price') }}</b></td>
            </tr>
            <tr>
                <td>商品缩略图</td>
                <td><img src="{{env('UPLOAD_URL')}}{{$goodsInfo->goods_img}}" width="50" height="50">
                    <input type="file" name="goods_img"></td>
            </tr>
            <tr>
                <td>商品库存</td>
                <td><input type="text" name="goods_num" value="{{$goodsInfo->goods_num}}"><b style="color:red">{{ $errors->first('goods_num') }}</b></td>
            </tr>
            <tr>
                <td>是否精品</td>
            ...
            <tr>
                ...
                <td><input type="radio" name="is_hot" value="1"  {{$goodsInfo->is_hot==1?'checked':''}}>是
                    <input type="radio" name="is_hot" value="2"  {{$goodsInfo->is_hot==2?'checked':''}}>否</td>
            </tr>
            <tr>
                <td>是否上架</td>
                <td><input type="radio" name="is_up" value="1" {{$goodsInfo->is_up==1?'checked':''}}>是
                    <input type="radio" name="is_up" value="2" {{$goodsInfo->is_up==2?'checked':''}}>否</td>
            </tr>
            <tr>
                <td>商品详情</td>
            ...
            <tr>
                ...
                <td>@if($goodsInfo->goods_imgs)
                        ...
                        <img src="{{env('UPLOAD_URL')}}{{$v}}" width="50px" height="50px">
                        @endforeach
                    @endif
                    <input type="file" name="goods_imgs[]" multiple="multiple"></td>
            </tr>
            <tr>
                <td>品牌</td>
                <td><select name="b_id">
            ...
            <tr>
                ...
                <td><select name="cate_id">
                        ...
                        @foreach($cateInfo as $v)
                            <option value="{{$v->cate_id}}" {{$goodsInfo->cate_id==$v->cate_id?'selected':''}}>{!! str_repeat('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;',$v->level*2.5) !!}{{$v->cate_name}}</option>
                        @endforeach
                    </select>
                </td>
            </tr>
