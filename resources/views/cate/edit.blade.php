...
<center>
    <form action="{{url('cate/update/'.$data->cate_id)}}" method="post">
        ...
        <table>
            <tr>
                <td>分类名称:</td>
                <td><input type="text" name="cate_name" placeholder="请输入分类名称" value="{{$data->cate_name}}"><b style="color:red">{{$errors->first('cate_name')}}
                    </b></td>
            </tr>
            <tr>
                <td>父级分类:</td>
                <td><select name="pid">
                        ...
                        @foreach($cateInfo as $k=>$v)
                            <option value="{{$v->cate_id}}"  @if($data->pid==$v->cate_id) selected @endif>{!! str_repeat("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;",$v->level*2.5)!!}{{$v->cate_name}}</option>
                        @endforeach
                    </select></td>
            </tr>
            <tr>
                <td>分类描述:</td>
                <td><textarea name="cate_detail" cols="30" rows="10">{{$data->cate_detail}}</textarea></td>
            </tr>
            <tr>
